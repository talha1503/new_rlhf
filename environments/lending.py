    ...
    if state.group_id is not None:
      ...
      if action:
        ...
        if state.total_loans_accepted[state.group_id] != 0:
          state.default_rates[state.group_id] = state.total_loans_defaulted[state.group_id]/state.total_loans_accepted[state.group_id]

      cluster_probability = state.params.applicant_distribution.components[state.group_id].weights
      state.average_credit_score[state.group_id] = sum([index * value for index, value in enumerate(cluster_probability)])
      state.timestep += 1
...
class State(core.State):
  ...
  acceptance_rates = attr.ib(default=[0,0])
  default_rates = attr.ib(default=[0,0])
