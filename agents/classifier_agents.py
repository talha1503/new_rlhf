  ...
  """
    ...
    def _act_impl(self, observation, reward, done):
        ...
        if self._step < self.params.burnin:
            ...
            score = self._score_transform([features])
            score = score[0]
...
class ClassifierAgent(ScoringAgent):
    ...
    include_summary_stats = attr.ib(None)
    state = attr.ib(None)
    classifier_name = attr.ib("logisitc")
    _classifier = attr.ib(None)
    
    def set_classifier(self):
        if self.classifier_name == "logistic":
            self._classifier = linear_model.LogisticRegression(solver="lbfgs")
    ...
    def _get_features(self, observation):
        ...
        acceptance_rate = self.state.acceptance_rates[group_id]
        default_rate = self.state.default_rates[group_id]
        average_credit_score = self.state.average_credit_score[group_id]
        current_credit_score = np.concatenate([
            observation.get(feature_key) for feature_key in self.params.feature_keys
        ], 0).ravel()
        if self.include_summary_stats:
            features = np.concatenate((current_credit_score,np.array([acceptance_rate,default_rate,average_credit_score])),axis=None)
        else:
            features = current_credit_score
        return features
